...
namespace Elwark.People.Background.TemplateViewEngine
{
    public class FluidRendering : IFluidRendering
    {
            ...
            ModelStateDictionary modelState)
        {
            ...
            context.FileProvider = new FileProviderMapper(fileProvider, "Templates");
        ...
        public IEnumerable<string> FindViewStarts(string viewPath, IFileProvider fileProvider)
        {
            ...
            while (!string.IsNullOrEmpty(viewPath) &&
                   !string.Equals(viewPath, "Templates", StringComparison.OrdinalIgnoreCase))
            {
                index = viewPath.LastIndexOf('/', index);

                if (index == -1) return viewStarts;
